...
interface PMTaskCalendarViewProps {
    tasks: ApiTask[];
...
export function PMTaskCalendarView({ tasks, onTaskClick }: PMTaskCalendarViewProps) {
                                    ...
                                    <div key={idx} className={cn("min-h-[130px] p-1.5 bg-white transition-colors cursor-pointer border-b border-slate-100", !isSameMonth(day, currentMonth) ? "bg-slate-50/40" : "hover:bg-blue-50/20")}>
                                        ...
                                        <div className="space-y-0.5">
                                            {dayTasks.slice(0, 3).map(task => { 
                                                ...
                                                return (
                                                    <div key={task.id} onClick={(e) => { e.stopPropagation(); onTaskClick(task); }} className={cn("flex items-center gap-1 px-1.5 py-1 rounded-[4px] text-[9px] font-bold truncate transition-all hover:shadow-sm", sColor.bg, sColor.text)}>
                                                        <span className={cn("h-1.5 w-1.5 rounded-full shrink-0", sColor.dot)} /><span>{task.title}</span>
                                                    </div>
                                                ); 
                                            })}
