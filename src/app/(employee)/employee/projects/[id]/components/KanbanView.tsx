...
import { Circle, PlayCircle, CheckCircle2, Calendar as CalendarIcon, MessageSquare, ArrowRight, Lock } from "lucide-react";
...
export function KanbanView({ projectId, tasks, members, onTaskClick, currentUser }: KanbanViewProps) {
    ...
    return (
        <div className="flex-1 lg:overflow-y-auto pr-1 pb-2 custom-scrollbar">
            <DragDropContext onDragEnd={onDragEnd}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {(["todo", "in_progress", "done"] as const).map(col => {
                        ...
                        return (
                            <div key={col} className="flex flex-col space-y-2 bg-slate-50/80 p-3 rounded-md border border-slate-100 min-h-[440px]">
                                                    ...
                                                    <Draggable key={task.id} draggableId={String(task.id)} index={index} isDragDisabled={!isAssignedToMe}>
                                                        {(provided, snapshot) => (
                                                            <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} style={provided.draggableProps.style}>
                                                                <Card 
                                                                    className={cn(
                                                                        "group bg-white border-slate-200 shadow-sm hover:shadow-md transition-all rounded-md overflow-hidden p-0 py-0 gap-0", 
                                                                ...
                                                                >
                                                                    <CardContent className="px-3 py-2.5 space-y-2 relative">
                                                                        {!isAssignedToMe && (
