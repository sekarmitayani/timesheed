...
import { FileText, Briefcase, Calendar, Info } from "lucide-react";
...
export function ContractOverview({ contracts }: ContractOverviewProps) {
  ...
  const renderContractList = (title: string, items: Contract[], icon: React.ReactNode) => (
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-4">
          {items.map((contract) => (
            <div
              key={contract.id}
              className="group relative flex flex-col md:flex-row md:items-center justify-between p-4 rounded-lg border border-[#E2E8F0] bg-white hover:border-[#4B7BEC]/30 transition-all"
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-sm">
                    {contract.project_name || (contract.project_id ? `Project #${contract.project_id}` : "Base Contract")}
                  </span>
              ...
              <div className="mt-4 md:mt-0 text-left md:text-right border-t md:border-t-0 pt-4 md:pt-0 border-[#E2E8F0]">
                ...
                <p className="text-sm font-bold text-[#4B7BEC]">
                  ...
                  <span className="text-[10px] text-muted-foreground font-normal ml-1">
                    / {contract.contract_type === 'monthly' ? 'mo' : contract.contract_type === 'mandays' ? 'day' : 'hr'}
                  </span>
                </p>
              </div>
            </div>
