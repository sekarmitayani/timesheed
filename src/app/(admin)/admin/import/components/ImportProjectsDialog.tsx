...
export function ImportProjectsDialog({ open, onOpenChange, onSuccess }: ImportProjectsDialogProps) {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            const result = await parseAndValidateProjectFile(file);
    ...
    const handleExecuteImport = async () => {
        ...
        setIsUploading(true);
        try {
            const csvFile = new File([csvBlob], "projects_import.csv", { type: "text/csv" });
            const result = await importService.importProjects(csvFile);
            setImportResult(result);
            setStep("result");

            if (result.success_count > 0) {
                toast.success(`Successfully imported ${result.success_count} projects!`);
                onSuccess(); // Triggers table / cards refresh
            } else if (result.skipped_count > 0 && result.error_count === 0) {
                toast.info("All project records already exist in the database (skipped).");
            } else {
                toast.error("No projects were imported due to validation errors.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to import project records");
        } finally {
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-lg bg-[#f8fafc] border border-[#e2e8f0]">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xs text-slate-500">
                                        Download our formatted Excel template with pre-configured headers for project details, clients, budgets, deadlines, and PM assignment.
                                    </p>
                                </div>
                                <Button
                                    type="button"
                            ...
                            <div className="flex items-start gap-2.5 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-900 text-xs">
                                ...
                                <div>
                                    ...
                                    <span className="ml-1 text-slate-700">
                                        The <strong>pm_email</strong> column is optional. If provided, the user must be already registered in the system with the <strong>projectmanager</strong> role. If blank or if the user is not a PM, the project will still be created without an assigned PM.
                                    </span>
                                </div>
                            </div>
                        </div>
                                ...
                                <table className="w-full text-left text-xs">
                                    <thead className="bg-slate-50 border-b border-[#e2e8f0] sticky top-0 z-10 text-slate-500 uppercase tracking-wider font-semibold text-[10px]">
                                        <tr>
                                            ...
                                            <th className="py-2.5 px-3">Planned Cost</th>
                                            <th className="py-2.5 px-3">Deadline</th>
                                            <th className="py-2.5 px-3">PM Email</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100 bg-white">
                                        {filteredRows.map((r) => (
                                            <tr key={r.rowIndex} className={r.isValid ? "hover:bg-slate-50/60" : "bg-rose-50/40 hover:bg-rose-50/80"}>
                                                ...
                                                <td className="py-2 px-3 font-semibold text-slate-800">{r.name || <span className="text-slate-300 italic">-</span>}</td>
                                                <td className="py-2 px-3 text-slate-700">{r.clientName || "-"}</td>
                                                <td className="py-2 px-3 font-semibold text-slate-800">{formatCurrency(r.budgetRevenue)}</td>
                                                <td className="py-2 px-3 font-semibold text-slate-800">{formatCurrency(r.budgetCost)}</td>
                                                <td className="py-2 px-3 text-slate-600 font-mono text-[11px]">{r.deadline || "-"}</td>
                                                <td className="py-2 px-3 font-mono text-[11px] text-slate-600">
                                                    {r.pmEmail ? (
                                                        <Badge variant="outline" className="text-[10px] bg-blue-50 text-blue-700 border-blue-200 font-normal">
                                                            {r.pmEmail}
                                                        </Badge>
                                                    ) : (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {isUploading ? (
                                        ...
                                        <>
                                            Import {validCount} Projects <ArrowRight className="h-3.5 w-3.5" />
                                        </>
                                    )}
                                </Button>
                            </div>
