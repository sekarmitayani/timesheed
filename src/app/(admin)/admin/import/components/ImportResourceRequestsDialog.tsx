...
import { downloadResourceRequestExcelTemplate, parseAndValidateResourceRequestFile, ParsedResourceRequestRow } from "@/lib/utils/excel-templates";
import { importService, ImportSummary } from "@/lib/services/import-service";
import { toast } from "sonner";

interface ImportResourceRequestsDialogProps {
    open: boolean;
...
export function ImportResourceRequestsDialog({ open, onOpenChange, onSuccess }: ImportResourceRequestsDialogProps) {
    const [step, setStep] = useState<Step>("upload");
    const [isParsing, setIsParsing] = useState(false);
    const [isUploading, setIsUploading] = useState(false);
    const [selectedFile, setSelectedFile] = useState<File | null>(null);
    const [csvBlob, setCsvBlob] = useState<Blob | null>(null);
    const [parsedRows, setParsedRows] = useState<ParsedResourceRequestRow[]>([]);
    ...
    const processFile = async (file: File) => {
        ...
        try {
            const result = await parseAndValidateResourceRequestFile(file);
    ...
    const handleExecuteImport = async () => {
        ...
        setIsUploading(true);
        try {
            const csvFile = new File([csvBlob], "resource_requests_import.csv", { type: "text/csv" });
            const result = await importService.importResourceRequests(csvFile);
            setImportResult(result);
            setStep("result");

            if (result.success_count > 0) {
                toast.success(`Successfully imported ${result.success_count} resource requests!`);
                onSuccess?.();
            } else if (result.skipped_count > 0 && result.error_count === 0) {
                toast.info("All resource requests already exist in the database (skipped).");
            } else {
                toast.error("No resource requests were imported due to validation errors.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to import resource requests");
        } finally {
    ...
    const renderTypeBadge = (type: string) => {
        switch (type) {
            case "tools":
                return <Badge className="bg-blue-50 text-blue-700 border-blue-200 text-[10px] font-semibold">Tools</Badge>;
            case "infrastructure":
                return <Badge className="bg-purple-50 text-purple-700 border-purple-200 text-[10px] font-semibold">Infra</Badge>;
            case "accommodation":
                return <Badge className="bg-amber-50 text-amber-700 border-amber-200 text-[10px] font-semibold">Accom</Badge>;
            case "manpower":
                return <Badge className="bg-teal-50 text-teal-700 border-teal-200 text-[10px] font-semibold">Manpower</Badge>;
            default:
                ...
                <div className="bg-[#f8fafc] border-b border-[#e2e8f0] px-6 py-4 pr-12 flex items-center justify-between">
                    <div>
                        ...
                        <DialogDescription className="text-xs text-slate-500 mt-0.5">
                            Bulk import tools, infrastructure, accommodation, and manpower requests
                        </DialogDescription>
                    </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-md bg-[#f8fafc] border border-[#e2e8f0]">
                                <div className="space-y-1">
                                    <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                        <Info className="h-3.5 w-3.5 text-[#2568C1]" /> Official Resource Request Template
                                    </h4>
                                    <p className="text-xs text-slate-500">
                                        Download our formatted Excel template with pre-configured headers for project name, requester email, resource type, details, and optional amounts.
                                    </p>
                                </div>
                                <Button
                                    type="button"
                                    variant="outline"
                                    size="sm"
                                    onClick={downloadResourceRequestExcelTemplate}
                            ...
                            <div className="flex items-start gap-2.5 p-3 rounded-md bg-blue-500/10 border border-blue-500/20 text-blue-900 text-xs">
                                ...
                                <div>
                                    ...
                                    <span className="ml-1 text-slate-700">
                                        Both <strong>project_name</strong> and <strong>requester_email</strong> must already exist in the system. <strong>type</strong> must be one of: <code>tools</code>, <code>infrastructure</code>, <code>accommodation</code>, <code>manpower</code>. Status defaults to <code>pending</code> if left blank.
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-md bg-[#f8fafc] border border-[#e2e8f0]">
                                <div className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className="text-slate-300">|</span>
                                    <span className="text-slate-600 font-medium">{parsedRows.length} total requests</span>
                                </div>
                                ...
                                <table className="w-full text-left text-xs">
                                    <thead className="bg-slate-50 border-b border-[#e2e8f0] sticky top-0 z-10 text-slate-500 uppercase tracking-wider font-semibold text-[10px]">
                                        <tr>
                                            ...
                                            <th className="py-2.5 px-3">Requester Email</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100 bg-white">
                                        {filteredRows.map((r) => (
                                            <tr key={r.rowIndex} className={r.isValid ? "hover:bg-slate-50/60" : "bg-rose-50/40 hover:bg-rose-50/80"}>
                                                ...
                                                <td className="py-2 px-3 font-mono text-[11px] text-slate-700">{r.requesterEmail || "-"}</td>
                    ...
                    {step === "result" && importResult && (
                        <div className="space-y-5">
                            ...
                            <div className="p-3 rounded-md bg-[#f8fafc] border border-[#e2e8f0] text-xs text-slate-600 flex items-center gap-2">
                                ...
                                <span>Resource requests have been registered in the system. Project managers and administrators have been notified.</span>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    {step === "upload" && (
                        <>
                            <div className="text-xs text-slate-400 font-medium">
                                Ensure project names and requester emails match registered data
                            </div>
                            <Button 
                                type="button" 
                                variant="ghost" 
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {isUploading ? (
                                        ...
                                        <>
                                            Import {validCount} Requests <ArrowRight className="h-3.5 w-3.5" />
                                        </>
                                    )}
                                </Button>
                            </div>
