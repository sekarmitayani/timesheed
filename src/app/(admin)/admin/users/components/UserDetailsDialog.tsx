...
export function UserDetailsDialog({
    ...
    const handleAddBaseRate = () => {
        onOpenChange(false);
        router.push(`/admin/contracts?newContract=true&userId=${user.id}`);
    };

    const handleViewContractInAdmin = (contractId: number) => {
        onOpenChange(false);
        router.push(`/admin/contracts?detailId=${contractId}`);
    };

    return (
        <Dialog open={open} onOpenChange={(val) => !isSavingContract && onOpenChange(val)}>
            <DialogContent className="sm:max-w-[1050px] w-[95vw] p-0 gap-0 overflow-hidden border-[#e2e8f0] bg-white rounded-2xl">
                {/* Header */}
                <div className="bg-[#f8fafc] border-b border-[#e2e8f0] px-6 py-4 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="text-sm flex gap-2 items-center mt-1">
                            ...
                            <Badge 
                                variant="outline" 
                                className="text-[10px] font-bold tracking-wider py-0.5 rounded-full bg-emerald-50 text-emerald-600 border-none px-2.5 capitalize"
                            >
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center pb-2 border-b border-[#e2e8f0]">
                                            ...
                                            <Button 
                                                size="sm" 
                                                variant="outline" 
                                                className="h-7 text-xs gap-1 border-[#2568C1] text-[#2568C1] hover:bg-blue-50 font-medium" 
                                                    ...
                                                    >
                                                        <div className="flex justify-between items-start">
                                                            <div className="flex flex-col gap-0.5">
                                                                <Badge variant="outline" className={`capitalize text-[10px] w-fit font-bold rounded-full px-2 py-0.5 border-none ${c.is_active ? 'bg-blue-50 text-[#2568C1]' : 'bg-slate-100 text-slate-500'}`}>
                                                        ...
                                                        <div className="text-sm font-extrabold text-[#0f172a] flex items-center justify-between mt-1">
                                                            <span className="bg-slate-50 py-0.5 px-2 rounded border border-slate-100 font-mono text-xs tracking-tight text-[#1e293b]">
                                                                Rp {(c.rate_amount || 0).toLocaleString('id-ID')}
                                                            </span>
                                                            {onDeleteContract && (
                                                                <Button 
                                                                    variant="ghost" 
                                                        ...
                                                        <div className="text-[10px] font-medium text-slate-500 flex items-center justify-between pt-1.5 border-t border-slate-100 mt-1">
                                                        ...
                                                        <div key={pid} className="border border-slate-200 rounded-lg overflow-hidden flex flex-col">
                                                            ...
                                                            <div className="p-4 bg-white space-y-3">
                                                                {pContracts.map(c => (
                                                                    <div 
                                                                        key={c.id} 
                                                                        className={`flex items-center justify-between p-3 rounded-md border ${c.is_active ? 'border-amber-200 bg-amber-50/30' : 'border-slate-100 bg-slate-50/50'} cursor-pointer hover:border-[#2568C1]/50`} 
                                                                        onClick={() => handleViewContractInAdmin(c.id)}
                                                                    >
