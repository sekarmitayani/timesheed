...
import { Loader2, ArrowLeft, Users, ListTodo, Package, Edit, Save, Trash2, DollarSign, Crown, UserPlus, Check, X } from "lucide-react";
            ...
            <Tabs defaultValue="overview" className="w-full">
                ...
                <TabsContent value="team" className="space-y-4 mt-0 focus:outline-none">
                    <div className="flex items-start justify-between mb-4">
                        ...
                        <Button size="sm" className="gap-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg shadow-sm h-9 px-4 shrink-0 mt-1" onClick={openAssignMember}>
                ...
                <TabsContent value="resources" className="space-y-4 mt-0 focus:outline-none">
                    <div className="mb-4">
                        ...
                        <p className="text-xs text-slate-500 mt-1">List of all approved resource requests and operational requirements for this project.</p>
                    </div>
                    {resources.filter(r => r.status === "approved").length === 0 ? (
                        <Card className="border-dashed border-slate-200 bg-slate-50 shadow-none">
                            <CardContent className="py-12 flex flex-col items-center justify-center text-center">
                                <Package className="h-10 w-10 text-slate-300 mb-3" />
                                <p className="text-sm font-medium text-slate-600">No approved resources for this project</p>
                            </CardContent>
                        </Card>
                    ) : (
                        <Card className="border-slate-200 shadow-sm rounded-xl overflow-hidden">
                                ...
                                <Table>
                                    <TableHeader className="bg-slate-50">
                                        <TableRow className="border-slate-200">
                                            ...
                                            <TableHead className="font-bold text-[11px] uppercase tracking-widest text-slate-500 h-11">Status</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {resources.filter(r => r.status === "approved").map((r, index) => (
                                            <TableRow key={r.id} className="border-slate-100 hover:bg-slate-50/50">
                                                <TableCell className="py-3 text-sm text-slate-500 font-medium text-center">{index + 1}</TableCell>
                                                <TableCell className="py-3 text-center uppercase text-xs font-medium text-slate-500">{r.type}</TableCell>
                                                <TableCell className="py-3 font-medium text-sm text-slate-900">{r.details}</TableCell>
