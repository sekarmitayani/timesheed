...
export function AdminProjectMembersDialog({
    ...
    return (
        <Dialog open={open} onOpenChange={o => !isSaving && onOpenChange(o)}>
            <DialogContent className="sm:max-w-[600px] max-w-[95vw] max-h-[90vh] p-0 gap-0 overflow-hidden border-[#e2e8f0] rounded-md shadow-xl bg-white flex flex-col">
                <div className="bg-[#f8fafc] border-b border-[#e2e8f0] px-6 pr-12 py-4 flex items-center justify-between shrink-0 gap-4">
                    <div className="min-w-0">
                        <DialogTitle className="text-base text-slate-900 font-bold truncate">Team Management</DialogTitle>
                        <DialogDescription className="text-xs text-slate-500 mt-0.5">{project?.name} • {members.length} members</DialogDescription>
                    </div>
                    {!showAssignForm && <Button size="sm" variant="outline" className="gap-1.5 text-xs h-8 px-3 border-[#2568C1] text-[#2568C1] hover:bg-[#2568C1]/10 rounded-md shrink-0" onClick={() => setShowAssignForm(true)}><UserPlus className="h-3.5 w-3.5" /> Add Member</Button>}
                </div>
                
                <div className="overflow-y-auto flex-1 custom-scrollbar">
                    {showAssignForm && (
                        <div className="border border-[#2568C1]/20 bg-[#2568C1]/5 rounded-md p-4 m-5 space-y-4 animate-in slide-in-from-top-2 duration-300">
                            <div className="flex items-center justify-between border-b border-[#2568C1]/10 pb-2 mb-2">
                                ...
                                <Button variant="ghost" size="icon" className="h-7 w-7 rounded-md" onClick={() => setShowAssignForm(false)}><X className="h-4 w-4" /></Button>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Select value={String(assignForm.user_id || "")} onValueChange={v => handleUserSelect(Number(v))}>
                                        <SelectTrigger className="h-9 bg-white rounded-md"><SelectValue placeholder="Choose" /></SelectTrigger>
                                        <SelectContent>
                                            {allUsers
                                                .filter(u => 
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input className="h-9 bg-white rounded-md" placeholder="e.g. Backend Dev" value={assignForm.role_in_project} onChange={e => setAssignForm({ ...assignForm, role_in_project: e.target.value })} />
                                </div>
                            </div>
                                ...
                                <div className="space-y-3 pt-2">
                                    ...
                                    {loadingContracts ? <div className="flex items-center gap-2 h-9 text-xs text-slate-400"><Loader2 className="h-3 w-3 animate-spin" /> Fetching contracts...</div> : 
                                        <Select value={selectedContractId} onValueChange={handleContractSelect}>
                                            <SelectTrigger className="h-9 bg-white rounded-md"><SelectValue placeholder="Select plan" /></SelectTrigger>
                                        ...
                                        <div className="pt-1 space-y-3 animate-in fade-in duration-300">
                                            <div className="grid grid-cols-2 gap-2">
                                                <div className="space-y-1">
                                                    ...
                                                    <CurrencyInput className="h-9 bg-white rounded-md" value={assignForm.custom_rate || ""} onChange={(v: any) => setAssignForm({ ...assignForm, custom_rate: Number(v) || null })} />
                                                </div>
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-bold text-slate-500 uppercase">Contract Type</label>
                                                    <Select value={assignForm.contract_type} onValueChange={v => setAssignForm({...assignForm, contract_type: v})}><SelectTrigger className="h-9 bg-white rounded-md"><SelectValue placeholder="Type" /></SelectTrigger><SelectContent><SelectItem value="timesheet">Timesheet</SelectItem><SelectItem value="mandays">Mandays</SelectItem><SelectItem value="monthly">Monthly</SelectItem><SelectItem value="yearly">Yearly</SelectItem></SelectContent></Select>
                                                </div>
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-bold text-slate-500 uppercase">Payment Scheme</label>
                                                    <Select value={assignForm.payment_scheme} onValueChange={v => setAssignForm({...assignForm, payment_scheme: v})}><SelectTrigger className="h-9 bg-white rounded-md"><SelectValue placeholder="Scheme" /></SelectTrigger><SelectContent><SelectItem value="monthly">Monthly</SelectItem><SelectItem value="back_to_back">B2B</SelectItem><SelectItem value="per_project">Per-Project</SelectItem><SelectItem value="daily">Daily</SelectItem></SelectContent></Select>
                                                </div>
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-bold text-slate-500 uppercase">Start Date</label>
                                                    <div className="relative">
                                                        <Input type="date" className="h-9 bg-white rounded-md text-xs w-full" value={assignForm.start_date ? assignForm.start_date.split('T')[0] : ""} onChange={(e) => setAssignForm({ ...assignForm, start_date: e.target.value ? new Date(e.target.value).toISOString() : new Date().toISOString() })} />
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="bg-amber-50 text-amber-700 text-[10px] p-2 rounded-md border border-amber-100 flex gap-2 items-start leading-tight">
                            ...
                            <Button className="w-full h-9 bg-[#2568C1] hover:bg-[#1a4f99] font-bold rounded-md" onClick={handleSave} disabled={isSaving}>{isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : "Confirm Assignment"}</Button>
                        </div>
                    )}
                    ...
                    <div className="px-5 py-3 space-y-2">
                        {isLoading ? <div className="py-12 flex justify-center"><Loader2 className="h-6 w-6 animate-spin text-[#2568C1]/40" /></div> : 
                         ...
                         members.map(m => (
                            <div key={m.id} className="flex items-center justify-between p-3 rounded-md border border-slate-100 hover:border-slate-200 bg-white transition-all group">
                ...
                <div className="px-6 py-3 border-t border-[#e2e8f0] bg-[#f8fafc] flex justify-end shrink-0">
