...
import { Search, RotateCcw, FolderX, ChevronLeft, ChevronRight } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { TrashProject } from "@/lib/services/trash-service";
import { RestoreItem } from "../hooks/useTrashData";
import { formatDate } from "@/lib/utils";

interface DeletedProjectsTabProps {
...
export function DeletedProjectsTab({
    projects,
    totalProjects,
    page,
    setPage,
    ...
    return (
        <div className="bg-white border border-slate-100 shadow-sm rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white">
                <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                        Deleted Projects ({totalProjects})
                    </span>
                </div>
            ...
            {/* Table Content */}
            {isLoading ? (
                <div className="p-12 text-center text-xs text-slate-400">Loading deleted projects...</div>
            ) : totalProjects === 0 ? (
                <div className="p-12 text-center flex flex-col items-center justify-center">
                    <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 mb-3">
                        <FolderX className="h-6 w-6" />
                    </div>
                    <p className="text-sm font-semibold text-slate-700">No deleted projects found</p>
                    <p className="text-xs text-slate-400 mt-1">
                        {searchQuery ? "No results match your search query." : "Trash is currently empty for projects."}
                    </p>
                </div>
            ) : (
                                    ...
                                    <td className="py-3 px-5 text-right">
                                        <Button
                                            ...
                                            onClick={() => onRestore({
                                                ...
                                                id: project.id,
                                                title: project.name,
                                                subtitle: `Client: ${project.client_name || "N/A"}`
                                            })}
            ...
            {/* Pagination Footer */}
            {!isLoading && totalPages > 0 && totalProjects > 0 && (
                <div className="border-t border-[#e2e8f0] bg-slate-50/30 px-5 py-3.5 flex items-center justify-between">
                    <div className="text-xs text-slate-500 font-medium">
                        Showing <span className="text-slate-900">{(page - 1) * limit + 1}</span> to <span className="text-slate-900">{Math.min(page * limit, totalProjects)}</span> of <span className="text-slate-900">{totalProjects}</span> projects
                    </div>
                    <div className="flex items-center gap-2">
                        <Button 
                            variant="outline" 
