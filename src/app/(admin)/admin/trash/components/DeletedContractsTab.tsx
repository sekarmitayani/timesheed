...
import { Search, RotateCcw, FileX, ChevronLeft, ChevronRight } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { TrashContract } from "@/lib/services/trash-service";
import { RestoreItem } from "../hooks/useTrashData";
import { formatRupiah, formatDate } from "@/lib/utils";

interface DeletedContractsTabProps {
...
export function DeletedContractsTab({
    contracts,
    totalContracts,
    page,
    setPage,
    ...
    return (
        <div className="bg-white border border-slate-100 shadow-sm rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white">
                <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                        Deleted Contracts ({totalContracts})
                    </span>
                </div>
                    ...
                    <div className="w-full sm:w-72">
                        <div className="p-0.5 rounded-lg">
                            <div className="relative">
                                ...
                                <Input
                                    placeholder="Search deleted contracts..."
                                    value={searchQuery}
            ...
            {/* Table Content */}
            {isLoading ? (
                <div className="p-12 text-center text-xs text-slate-400">Loading deleted contracts...</div>
            ) : totalContracts === 0 ? (
                <div className="p-12 text-center flex flex-col items-center justify-center">
                    <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 mb-3">
                        <FileX className="h-6 w-6" />
                    </div>
                    <p className="text-sm font-semibold text-slate-700">No deleted contracts found</p>
                    <p className="text-xs text-slate-400 mt-1">
                        {searchQuery ? "No results match your search query." : "Trash is currently empty for contracts."}
                    </p>
                </div>
            ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-100 bg-slate-50/60 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                                ...
                                <th className="py-3 px-4">Type &amp; Scheme</th>
                                ...
                                <tr key={contract.id} className="hover:bg-slate-50/70 transition-colors">
                                    ...
                                    <td className="py-3 px-4">
                                        <span className="font-medium text-slate-700">
                                            {contract.project_name || "Global Base Rate"}
                                        </span>
                                    </td>
                                    <td className="py-3 px-4">
                                    ...
                                    <td className="py-3 px-4 font-mono font-medium text-slate-800">
                                        {formatRupiah(contract.rate_amount)}
                                    </td>
                                    <td className="py-3 px-4 text-slate-500 text-[11px]">
                                        {contract.deleted_at ? formatDate(contract.deleted_at) : "—"}
                                    </td>
                                    <td className="py-3 px-5 text-right">
                                        <Button
                                            ...
                                            onClick={() => onRestore({
                                                type: "contracts",
                                                id: contract.id,
                                                title: `Contract for ${contract.user_name || `User #${contract.user_id}`}`,
                                                subtitle: `Scope: ${contract.project_name || "Global"} • ${formatRupiah(contract.rate_amount)}`
                                            })}
            ...
            {/* Pagination Footer */}
            {!isLoading && totalPages > 0 && totalContracts > 0 && (
                <div className="border-t border-[#e2e8f0] bg-slate-50/30 px-5 py-3.5 flex items-center justify-between">
                    <div className="text-xs text-slate-500 font-medium">
                        Showing <span className="text-slate-900">{(page - 1) * limit + 1}</span> to <span className="text-slate-900">{Math.min(page * limit, totalContracts)}</span> of <span className="text-slate-900">{totalContracts}</span> contracts
                    </div>
                    <div className="flex items-center gap-2">
                        <Button 
                            variant="outline" 
